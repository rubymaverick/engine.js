var vm = function(){
  ...
  function runInContext(code, sandbox, filename){
    if (Object(sandbox) === sandbox) {
      if (!contexts.has(sandbox)) {
        createContext(sandbox);
      }
      return contexts.get(sandbox).run(code, filename);
    } else {
...
ContextifyVm.prototype.execute = function(code, sandbox, async){
  ...
  try {
    response.evaluation = vm.runInContext("'use strict'; " + code, ctx, "placeholder.js");
  } catch (e) {
    response.evaluation = e.stack;
    response.error = true;
  }
